...
/***************************************************************************/
/*! Modificado por Ricardo Márquez Turiello para ser llamada desde fortran */
/*! 19/05/2016                                                             */
/***************************************************************************/
#include "metisbin.h"

void computefillin_f2c_(idx_t *nvtxs, idx_t *nedges, idx_t *xadj, 
          idx_t *adjncy, idx_t *perm, idx_t *iperm)
{
  ...
  argv[0]=(char*)"ComputeFillIn_f2c";
  argv[1]=(char*)"metis_grafo.dat";
...
//  graph = ReadGraph(params);
  graph = CreateGraph();
  graph->nvtxs=*nvtxs;
  graph->nedges=(*nedges)*2;
  graph->ncon = 1;
  graph->xadj=xadj;
  graph->adjncy=adjncy;
//  gk_stopcputimer(params->iotimer);
//  printf("\n%d %d %d %d %d %d %d\n",graph->nvtxs,graph->nedges,graph->ncon,graph->xadj[0],graph->xadj[graph->nvtxs],graph->adjncy[0],graph->adjncy[graph->xadj[graph->nvtxs]-1]);
...
//      /* Write the solution */
//      gk_startcputimer(params->iotimer);
//      WritePermutation("metis_perm.dat", perm, graph->nvtxs); 
//      WritePermutation("metis_iperm.dat", iperm, graph->nvtxs); 
//      gk_stopcputimer(params->iotimer);
//    }
//
    CFIReportResults(params, graph, perm, iperm);
//  }
...
/*************************************************************************/
void CFIPrintInfo(params_t *params, graph_t *graph)
{ 
  char SVNINFO[]="ife3D";
  printf("******************************************************************************\n");
  printf("%s", METISTITLE);
  ...
  printf(" #Vertices: %"PRIDX", #Edges: %"PRIDX"\n", 
...
/*************************************************************************/
void CFIReportResults(params_t *params, graph_t *graph, idx_t *perm, 
         idx_t *iperm)
{ 
  size_t maxlnz, opc;
