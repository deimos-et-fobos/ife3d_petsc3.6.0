...
graph_t *ReadGraph(params_t *params)
{
  ...
  if (!gk_fexists(params->filename)) 
    errexit((char*)"File %s does not exist!\n", params->filename);

  graph = CreateGraph();

  fpin = gk_fopen(params->filename, (char*)"r", "ReadGRaph: Graph");

  /* Skip comment lines until you get to the first valid line */
  do {
    if (gk_getline(&line, &lnlen, fpin) == -1) 
      errexit((char*)"Premature end of input file: file: %s\n", params->filename);
  } while (line[0] == '%');
  ...
  if (nfields < 2) 
    errexit((char*)"The input file does not specify the number of vertices and edges.\n");

  if (graph->nvtxs <= 0 || graph->nedges <= 0) 
    errexit((char*)"The supplied nvtxs:%"PRIDX" and nedges:%"PRIDX" must be positive.\n", 
        graph->nvtxs, graph->nedges);
        
  if (fmt > 111) 
    errexit((char*)"Cannot read this type of file format [fmt=%"PRIDX"]!\n", fmt);
  ...
  xadj   = graph->xadj   = ismalloc(graph->nvtxs+1, 0, (char*)"ReadGraph: xadj");
  adjncy = graph->adjncy = imalloc(graph->nedges, (char*)"ReadGraph: adjncy");
  vwgt   = graph->vwgt   = ismalloc(ncon*graph->nvtxs, 1, (char*)"ReadGraph: vwgt");
  adjwgt = graph->adjwgt = ismalloc(graph->nedges, 1, (char*)"ReadGraph: adjwgt");
  vsize  = graph->vsize  = ismalloc(graph->nvtxs, 1, (char*)"ReadGraph: vsize");
  ...
  for (xadj[0]=0, k=0, i=0; i<graph->nvtxs; i++) {
    do {
      if (gk_getline(&line, &lnlen, fpin) == -1) 
        errexit((char*)"Premature end of input file while reading vertex %"PRIDX".\n", i+1);
    } while (line[0] == '%');
    ...
    if (readvs) {
      ...
      if (newstr == curstr)
        errexit((char*)"The line for vertex %"PRIDX" does not have vsize information\n", i+1);
      if (vsize[i] < 0)
        errexit((char*)"The size for vertex %"PRIDX" must be >= 0\n", i+1);
      curstr = newstr;
    }
    ...
    if (readvw) {
      for (l=0; l<ncon; l++) {
        ...
        if (newstr == curstr)
          errexit((char*)"The line for vertex %"PRIDX" does not have enough weights "
                  "for the %"PRIDX" constraints.\n", i+1, ncon);
        if (vwgt[i*ncon+l] < 0)
          errexit((char*)"The weight vertex %"PRIDX" and constraint %"PRIDX" must be >= 0\n", i+1, l);
        curstr = newstr;
      }
    }
    ...
    while (1) {
      ...
      if (edge < 1 || edge > graph->nvtxs)
        errexit((char*)"Edge %"PRIDX" for vertex %"PRIDX" is out of bounds\n", edge, i+1);

      ewgt = 1;
      if (readew) {
        ewgt = strtoidx(curstr, &newstr, 10);
        if (newstr == curstr)
          errexit((char*)"Premature end of line for vertex %"PRIDX"\n", i+1);
        if (ewgt <= 0)
          errexit((char*)"The weight (%"PRIDX") for edge (%"PRIDX", %"PRIDX") must be positive.\n", 
              ewgt, i+1, edge);
        curstr = newstr;
      }

      if (k == graph->nedges)
        errexit((char*)"There are more edges in the file than the %"PRIDX" specified.\n", 
            graph->nedges/2);
...
mesh_t *ReadMesh(params_t *params)
{
  ...
  fpin = gk_fopen(params->filename, (char*)"r", __func__);
  ...
  if (nfields < 1) 
    errexit((char*)"The input file does not specify the number of elements.\n");

  if (mesh->ne <= 0) 
    errexit((char*)"The supplied number of elements:%"PRIDX" must be positive.\n", mesh->ne);
        
  if (mesh->ne > nlines)
    errexit((char*)"The file has %zu lines which smaller than the number of "
            "elements of %"PRIDX" specified in the header line.\n", nlines, mesh->ne);

  ncon = mesh->ncon;
  eptr = mesh->eptr = ismalloc(mesh->ne+1, 0, (char*)"ReadMesh: eptr");
  eind = mesh->eind = imalloc(ntokens, (char*)"ReadMesh: eind");
  ewgt = mesh->ewgt = ismalloc((ncon == 0 ? 1 : ncon)*mesh->ne, 1, (char*)"ReadMesh: ewgt");
  ...
  for (eptr[0]=0, k=0, i=0; i<mesh->ne; i++) {
    do {
      if (gk_getline(&line, &lnlen, fpin) == -1) 
        errexit((char*)"Premature end of input file while reading element %"PRIDX".\n", i+1);
    } while (line[0] == '%');
    ...
    for (l=0; l<ncon; l++) {
      ...
      if (newstr == curstr)
        errexit((char*)"The line for vertex %"PRIDX" does not have enough weights "
                "for the %"PRIDX" constraints.\n", i+1, ncon);
      if (ewgt[i*ncon+l] < 0)
        errexit((char*)"The weight for element %"PRIDX" and constraint %"PRIDX" must be >= 0\n", i+1, l);
      curstr = newstr;
    }
    ...
    while (1) {
      ...
      if (node < 1)
        errexit((char*)"Node %"PRIDX" for element %"PRIDX" is out of bounds\n", node, i+1);
...
void ReadTPwgts(params_t *params, idx_t ncon)
{
  ...
  params->tpwgts = rsmalloc(params->nparts*ncon, -1.0, (char*)"ReadTPwgts: tpwgts");
  ...
  if (!gk_fexists(params->tpwgtsfile)) 
    errexit((char*)"Graph file %s does not exist!\n", params->tpwgtsfile);

  fpin = gk_fopen(params->tpwgtsfile, (char*)"r", "ReadTPwgts: tpwgtsfile");

  while (gk_getline(&line, &lnlen, fpin) != -1) {
    gk_strchr_replace(line, (char*)" ", (char*)"");
    ...
    if (newstr == curstr)
      errexit((char*)"The 'from' component of line <%s> in the tpwgts file is incorrect.\n", line);
    curstr = newstr;

    if (curstr[0] == '-') {
      to = strtoidx(curstr+1, &newstr, 10);
      if (newstr == curstr)
        errexit((char*)"The 'to' component of line <%s> in the tpwgts file is incorrect.\n", line);
      curstr = newstr;
    }
    else {
      to = from;
    ...
    if (curstr[0] == ':') {
      ...
      if (newstr == curstr)
        errexit((char*)"The 'fromcnum' component of line <%s> in the tpwgts file is incorrect.\n", line);
      curstr = newstr;

      if (curstr[0] == '-') {
        tocnum = strtoidx(curstr+1, &newstr, 10);
        if (newstr == curstr)
          errexit((char*)"The 'tocnum' component of line <%s> in the tpwgts file is incorrect.\n", line);
        curstr = newstr;
      }
      else {
        tocnum = fromcnum;
    ...
    if (curstr[0] == '=') {
      ...
      if (newstr == curstr)
        errexit((char*)"The 'wgt' component of line <%s> in the tpwgts file is incorrect.\n", line);
      curstr = newstr;
    }
    else {
      errexit((char*)"The 'wgt' component of line <%s> in the tpwgts file is missing.\n", line);
    }
    ...
    if (from < 0 || to < 0 || from >= params->nparts || to >= params->nparts)
      errexit((char*)"Invalid partition range for %"PRIDX":%"PRIDX"\n", from, to);
    if (fromcnum < 0 || tocnum < 0 || fromcnum >= ncon || tocnum >= ncon)
      errexit((char*)"Invalid constraint number range for %"PRIDX":%"PRIDX"\n", 
          fromcnum, tocnum);
    if (awgt <= 0.0 || awgt >= 1.0)
      errexit((char*)"Invalid partition weight of %"PRREAL"\n", awgt);
    for (i=from; i<=to; i++) {
  ...
  for (j=0; j<ncon; j++) {
    ...
    if (nleft > 0) {
      if (twgt > 1)
        errexit((char*)"The total specified target partition weights for constraint #%"PRIDX
...
void ReadPOVector(graph_t *graph, char *filename, idx_t *vector)
{
  ...
  fpin = gk_fopen(filename, (char*)"r", __func__);
  for (i=0; i<graph->nvtxs; i++) {
    if (fscanf(fpin, "%"SCIDX"\n", vector+i) != 1)
      errexit((char*)"[%s] Premature end of file %s at line %d [nvtxs: %d]\n",
...
void WritePartition(char *fname, idx_t *part, idx_t n, idx_t nparts)
{
  ...
  fpout = gk_fopen(filename, (char*)"w", __func__);
...
//  char filename[MAXLINE];
  ...
  fpout = gk_fopen(fname, (char*)"w", __func__);
