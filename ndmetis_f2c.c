...
void ndmetis_f2c_(idx_t *nvtxs, idx_t *nedges, idx_t *xadj, 
...
//  iperm = imalloc(graph->nvtxs, "main: iperm");
  ...
  if (status != METIS_OK) {
    ...
    if (!params->nooutput) {
      /* Write the solution */
      gk_startcputimer(params->iotimer);
      WritePermutation((char*)"metis_perm.dat", perm, graph->nvtxs); 
      WritePermutation((char*)"metis_iperm.dat", iperm, graph->nvtxs); 
      gk_stopcputimer(params->iotimer);
    }
