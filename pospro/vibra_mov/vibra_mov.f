************************************************************************
*     Movimiento de la placa para Paraview
*     
*     By Richi-07/09/15
************************************************************************
      program vibra_mov
...
*
        do 20 i=1,nper*pasos+1
          StTime = Str4(i-1)
          filemov=trim(file_raiz)//StEv//'_'//StTime//trim(file_ext)
 ...
 21       continue
          ...
          do 22 l=1,ndatos      !Datos
            write(2,rec=l+20) real(sol(l)*dsin(frec(j)*time(i)*2*pi))
 22       continue 
          close(2)
 20     continue
...
*
        filecase = trim(file_raiz)//StEv//'.case'
     ...
     &    '    '//trim(file_raiz)//StEv//'_****'//trim(file_ext)
        ...
        if(ivec.ne.0)then
          write(27,'(a)')  'vector per node:     '//trim(header)//
     &    '    '//trim(file_raiz)//StEv//'_****'//trim(file_ext)
        endif
        write(27,'(a)')  ''
        write(27,'(a)')  'TIME'
